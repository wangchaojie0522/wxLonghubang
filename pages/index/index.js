    ...
    GetData({
      ...
      if (res.code == 20000) {
        ...
        let arr = arr1.map(item=>{
          let num = (Number(item.buyAmount) - Number(item.sellAmount))/10000
          return {
            chg: Number(item.chg).toFixed(2) + '',
            name: item.name,
            thsCode: item.thsCode,
            amount: ((num>10000 || num < -10000) ? (num/10000).toFixed(2) + '亿' : num.toFixed(1) + '万')
          }
        })
        console.log(arr)
        thiz.setData({
